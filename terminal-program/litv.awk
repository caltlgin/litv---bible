BEGIN {
	...
	MAX_WIDTH = 80
	if (ENVIRON["LITV_MAX_WIDTH"] ~ /^[0-9]+$/) {
		if (int(ENVIRON["LITV_MAX_WIDTH"]) < MAX_WIDTH) {
			MAX_WIDTH = int(ENVIRON["LITV_MAX_WIDTH"])
		}
	}
...
function printverse(verse,    word_count, characters_printed) {
	if (ENVIRON["LITV_NOLINEWRAP"] != "" && ENVIRON["LITV_NOLINEWRAP"] != "0") {
		printf("%s\n", verse)
		return
	}
