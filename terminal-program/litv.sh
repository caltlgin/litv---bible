...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data litv.tsv | awk -v cmd=list "$(get_data litv.awk)"
		exit
	elif [ "$1" = "-W" ]; then
		export LITV_NOLINEWRAP=1
		shift
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data litv.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data litv.awk)" | ${PAGER}
	done
	exit 0
fi

get_data litv.tsv | awk -v cmd=ref -v ref="$*" "$(get_data litv.awk)" | ${PAGER}
